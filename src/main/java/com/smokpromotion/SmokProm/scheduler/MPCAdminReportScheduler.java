...
@Service
@Profile({"admin"})
public class MPCAdminReportScheduler implements Runnable {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(MPCAdminReportScheduler.class);
    ...
    private final Map<Integer, AdminScheduleTask> inProgressMap;
    ...
    private AdminScheduleServices scheduleServices;

    @Autowired
    MPCAdminReportScheduler(DR_AdminSchedule drSchedule,
                            DR_AdminScheduleLogEntry drScheduleLogEntry,
                            AdminScheduleMetrics scheduleMetrics,
                            ...
                            @Value("${MPC_ADMIN_SCHEDULER_RUN:false}") String runScheduler,
                            @Value("${MPC_ADMIN_SCHEDULER_MAX_RUNTIME:1800}") int timeout,
                            @Value("${MPC_ADMIN_SCHEDULER_TIMEZONE:UTC}") String timezone,
                            @Value("${MPC_ADMIN_SCHEDULER_SLOW_MAX_THREADS:1}") int slowPoolSize,
                            @Value("${MPC_ADMIN_SCHEDULER_FAST_MAX_THREADS:1}") int fastPoolSize,
                            @Value("${MPC_ADMIN_SCHEDULER_CUTOFF_FOR_FAST_SECONDS:300}") int estimateCutOffForFast,
                            MPCAppDBConnectionFactory dbFactory
    ) {

        this.running = false;
        this.queue = new ConcurrentLinkedQueue<DE_Schedule>();
        this.inProgressMap = new HashMap<Integer,AdminScheduleTask>();
        ...
        try {
            ...
            throw new RuntimeException("MPC_ADMIN_SCHEDULER_TIMEZONE not a legal timezone ",e);
        }
        scheduleServices = new AdminScheduleServices(drSchedule, drScheduleLogEntry, timezone, dbFactory, drSchedulePermissions, context, principleService
...
// Run every minute to ten seconds timeout
    ...
    public void timeout() {
        for(AdminScheduleTask schedule : inProgressMap.values()) {
    ...
    protected boolean scheduleFilter(DE_Schedule report, LocalDateTime now) {
    ...
    public Map<Integer, AdminScheduleTask> getInProgressMap(){
    ...
    public void run() {
        ...
        while (running) {
            ...
            synchronized (lock) {
                ...
                AdminScheduleTask task = new AdminScheduleTask(report, scheduleServices,inProgressMap, scheduleMetrics);
