...
@Service
public class DR_Schedule
{
    private static final Logger LOGGER = LoggerFactory.getLogger(DR_Schedule.class);

    private static final String URC_EMAIL = "@urcompliant.com";

    private final MPCAppDBConnectionFactory dbFactory;
    private ObjectMapper mapper;
    private static final String TABLE_NAME  = "schedule_rpts";
    ...
    public List<DE_ScheduleWithLastLog> allForUser(PortalEnum portal, String username) {
        try {
        return dbFactory.getJdbcTemplate(portal).map(template->template.query(
                "SELECT sched.id, sched.practiceGroupId, sched.recipients, sched.active, sched.portal,sched.username,sched.dest_email,sched.firstname,sched.lastname,sched.reportname, sched.zip_pass, sched.schedule, \n" +
                        ...
                        "FROM "+TABLE_NAME+" sched \n"+
                         " LEFT JOIN "+DR_ScheduleLogEntry.TABLE_NAME+" sl ON sl.schedule_id=sched.id AND sched.last_started=sl.start_time AND sched.deleted=0\n"+
        ...
        try {
            return dbFactory.getJdbcTemplate(portal).map(template->template.query(
                    "SELECT sched.id, sched.practiceGroupId, sched.recipients, sched.active, sched.portal, sched.username,sched.dest_email,sched.firstname,sched.lastname,sched.reportname, sched.zip_pass, sched.schedule, \n" +
                            ...
                            " sl.id as log_id, sl.schedule_id, sl.start_time, sl.runtime, sl.status as status,sl.attachment_size as attachment_size, sl.exception_class, sl.exception_message, sl.exception_stacktrace, sl.cause_class, sl.cause_message, sl.cause_stacktrace\n" +
                            "FROM "+TABLE_NAME+" sched \n" +
                            " LEFT JOIN "+DR_ScheduleLogEntry.TABLE_NAME+" sl ON sl.schedule_id=sched.id AND sched.last_started=sl.start_time \n" +
                            "ORDER BY sched.id DESC\n",
                    new Object[]{},
                    new DeScheduleMapperWithLastLog())).orElse(new LinkedList<>());
        } catch (DataAccessException e){
    ...
    public List<DE_ScheduleWithLastLog> getByDateTimesAndGroupNameForAdmin(PortalEnum portal, String groupName, LocalDateTime start, LocalDateTime end) {
        try {
            return dbFactory.getJdbcTemplate(portal).map(template->template.query(
                    "SELECT sched.id, sched.practiceGroupId, sched.recipients, pg.PracticeGroupName, sched.active, sched.portal, sched.username,sched.dest_email,sched.firstname,sched.lastname,sched.reportname, sched.zip_pass, sched.schedule, \n" +
                            " sched.day_of_week, sched.day_of_month, sched.month_of_year,sched.minutes_of_hour, sched.hour_of_day, sched.lastrun, sched.class, sched.method, sched.report_criteria_form, sched.start_date_days_ago, sched.created_datetime, " +
                            " sched.deleted,\n " +
                            " sl.id as log_id, sl.schedule_id, sl.start_time, sl.runtime, sl.status as status,sl.attachment_size as attachment_size, sl.exception_class, sl.exception_message, sl.exception_stacktrace, sl.cause_class, sl.cause_message, sl.cause_stacktrace\n" +
                            "FROM "+TABLE_NAME+" sched \n" +
                            ...
                            " LEFT JOIN "+DR_ScheduleLogEntry.TABLE_NAME+" sl ON sl.schedule_id=sched.id AND sched.last_started=sl.start_time \n" +
                            " WHERE "+(groupName.equals("")? " ?='%%' " : " pg.PracticeGroupName like ?")+" AND ((sched.lastrun BETWEEN ? AND ?) OR (sched.created_datetime BETWEEN ? AND ?))\n"+
                            "ORDER BY sched.id DESC\n",
    ...
    public List<DE_ScheduleWithLastLog> getByDateTimesAndGroupIdsForAdminAndPortalDC(PortalEnum portal, List<Integer> groupIds, LocalDateTime start, LocalDateTime end) {
        try {
            return dbFactory.getJdbcTemplate(portal).map(template->template.query(
                    "SELECT sched.id, sched.practiceGroupId, sched.recipients, '' as PracticeGroupName, sched.active, sched.portal, sched.username,sched.dest_email,sched.firstname,sched.lastname,sched.reportname, sched.zip_pass, sched.schedule, \n" +
                            " sched.day_of_week, sched.day_of_month, sched.month_of_year,sched.minutes_of_hour, sched.hour_of_day, sched.lastrun, sched.class, sched.method, sched.report_criteria_form, sched.start_date_days_ago, sched.created_datetime, " +
                            " sched.deleted,\n " +
                            " sl.id as log_id, sl.schedule_id, sl.start_time, sl.runtime, sl.status as status,sl.attachment_size as attachment_size, sl.exception_class, sl.exception_message, sl.exception_stacktrace, sl.cause_class, sl.cause_message, sl.cause_stacktrace\n" +
                            "FROM "+TABLE_NAME+" sched \n" +
                            " LEFT JOIN "+DR_ScheduleLogEntry.TABLE_NAME+" sl ON sl.schedule_id=sched.id AND sched.last_started=sl.start_time \n" +
                            " WHERE sched.portal='DC' AND sched.practiceGroupId IN ("+groupIds.stream().map(x->Integer.toString(x)).collect(Collectors.joining(","))+") AND ((sched.lastrun BETWEEN ? AND ?) OR (sched.created_datetime BETWEEN ? AND ?))\n"+
                            "ORDER BY sched.id DESC\n",
                    new Object[]{ Timestamp.valueOf(start), Timestamp.valueOf(end), Timestamp.valueOf(start), Timestamp.valueOf(end)},
                    new DeScheduleMapperWithLastLog())).orElse(new LinkedList<>());
        } catch (DataAccessException e){
            LOGGER.warn("Data Access Exception reading table",e);
            return new LinkedList<>();
    ...
    public LocalDateTime scheduleReport(PortalEnum portal, PortalSecurityPrinciple principle, ReportCriteriaForm form, Class controller, String method) {
        ...
        if (portal == null) {
            ...
            try {
                String sql = "INSERT INTO " + TABLE_NAME +
                        " (active,username,practiceGroupId, recipients, portal, reportname, firstname, lastname , dest_email, schedule, day_of_week, day_of_month,month_of_year, hour_of_day, minutes_of_hour,class,method,report_criteria_form, zip_pass, created_datetime, deleted) values "+
                        "(:active, :username, :practiceGroupId, :recipients, :portal, :reportname, :firstname, :lastname, :dest_email, :schedule, :day_of_week, :day_of_month, :month_of_year, :hour_of_day, :minute_of_hour, :class, :method, :report_criteria_form, :zip_pass, now(), 0);";
    ...
    protected class ScheduleParameterSourceCreator {
        ...
        public ScheduleParameterSourceCreator(PortalSecurityPrinciple principle, ReportCriteriaForm form, Class theClass, String method, LocalDateTime created, ObjectMapper mapper){
        ...
        public SqlParameterSource getParameterSource() throws SQLException{
            ...
            paramMap.put("practiceGroupId", principle.getPracticeGroupId());
            paramMap.put("portal", principle.getPortal().name());
            ...
            paramMap.put("recipients", form.getRecipients());
    ...
    public ScheduleParameterSourceCreator getScheduleStatementCreator( PortalSecurityPrinciple principle, ReportCriteriaForm form, Class theClass, String method,  LocalDateTime created){
    ...
    public DeScheduleMapper getScheduleMapper(){
    ...
    protected class DeScheduleMapper implements RowMapper<DE_Schedule> {
        ...
        public DE_Schedule mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            schedule.setPracticeGroupId(rs.getInt("practiceGroupId"));
    ...
    protected class DeScheduleMapperWithLastLog implements RowMapper<DE_ScheduleWithLastLog> {
        ...
        public DE_ScheduleWithLastLog mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            try {
                schedule.setPracticeGroupId(rs.getInt("practiceGroupId"));
            } catch (Exception e){} // Avoid Null point with empty group id
            try {
                schedule.setPracticeGroupName(rs.getString("PracticeGroupName"));
            } catch (Exception e){} // Avoid Null point with empty group name
            schedule.setActive(rs.getBoolean("active"));
            schedule.setUsername(rs.getString("username"));
            schedule.setPortal(rs.getString("portal"));
            schedule.setReportname(rs.getString("reportname"));
